...
class HoneypotSimulator:
    """
    A class to simulate different types of connections and attacks against the honeypot.
    ...
    def __init__(self,target_ip="127.0.0.1", intensity="medium"):
        ...
        #Common ports that hackers probe
        ...
        #Intensify settings affect the frequency and volume of simulated attacks
    ...
    def simulate_connection(self, port):
        ...
        try:
            ...
            print(f"[+] Recived banner from port {port}: {banner.decode('utf-8', 'ignore').strip()}")
            ...
            if port in self.attack_patterns:
                for command in self.attack_patterns[port]:
                    ...
                    try:
                        ...
                        print(f"[+] Recived response: {response.decode('utf-8', 'ignore').strip()}")
                    except socket.timeout:
                        print(f"[-] No response recived from port {port}")
                    
                    #Add realistic dely between commands
    ...
    def run_continous_simulation(self, duration=300):
        """
        Runs a continous simulattion for a specified duration
        """  
        print(f"\n[*] Starting contious simulation for {duration} seconds")
        ...
        with ThreadPoolExecutor(
            ...
            while time.time() < end_time:
                # Mix of diffrent attack patterns
                simulation_choices = [
...
def main():
    ...
    try:
        simulator.run_continous_simulation(args.duration)
